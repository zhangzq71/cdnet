...
static inline int spi_mem_write(spi_t *spi, uint8_t mem_addr, const uint8_t *buf, int len)
{
	...
	spi->hspi->ctrl1_bit.spien = TRUE;
	spi->ns_pin->group->clr = spi->ns_pin->num;
	while (!(spi->hspi->sts & SPI_I2S_TDBE_FLAG));
	spi->hspi->dt = mem_addr;
	while (!(spi->hspi->sts & SPI_I2S_RDBF_FLAG));
	dummy_data = spi->hspi->dt;
	...
	spi->ns_pin->group->scr = spi->ns_pin->num;
...
static inline int spi_mem_read(spi_t *spi, uint8_t mem_addr, uint8_t *buf, int len)
{
	...
	spi->hspi->ctrl1_bit.spien = FALSE;
